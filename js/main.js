...
var maxAppearEnemy = 0;//屏幕上一起出现的最大数
...
$(document).keydown(function(e){
	...
	case GAME_STATE_START:
		...
		//射击
		if(e.keyCode == keyboard.SPACE && player1.lives > 0 && player1.isShooting <= 5){
			player1.shoot();
		}else if(e.keyCode == keyboard.ENTER && player2.lives > 0 && player1.isShooting <= 5){
			player2.shoot();
	...
	if(bulletArray != null && bulletArray.length > 0){
		for(var i=0;i<bulletArray.length;i++){
			...
			if(bulletObj.isDestroyed){
				bulletObj.owner.isShooting = bulletObj.owner.isShooting - 1;
