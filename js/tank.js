...
var Tank = function(){
	...
	this.shoot = function(){
		...
		if(this.isShooting >=5 ){
			return ;
		}else{
			var tempX = this.x;
			var tempY = this.y;
			this.bullet = new Bullet(this.ctx,this,this.dir);
			if(this.dir == UP){
			...
			//保证子弹不出边界
			if(tempX <= map.offsetX){
				tempX = map.offsetX + 1;
			}
			else if(tempX >= map.offsetX + map.mapWidth - this.size){
				tempX = map.offsetX + map.mapWidth - this.size;
			}
			if(tempY <= map.offsetY ){
				tempY = map.offsetY + 1;
			}else if(tempY >= map.offsetY + map.mapHeight + 1){
				tempY = map.offsetY + map.mapHeight - this.size/2;
			}
			...
			this.isShooting = this.isShooting+1;
		}
	};
